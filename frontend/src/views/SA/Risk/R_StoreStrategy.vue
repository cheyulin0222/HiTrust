<template>
	...
	<template v-if = "showType === 'SEARCH' ">
		<div class="row p-3 justify-content-md-center" ref = 'search' >
			<CardItem colMd="7">
				<SelectView :options="merIdOption" :invalidTxt="invalids.merId" LabelVal = "商店 ID"
				v-model="merId" ref = "merId"/>
			</CardItem>
		</div>
	...
	<template v-else if = " showType === 'EDIT' ">
		<div class="col-md-12 pr-0 text-center font-weight-bold t-18">商店 ID:{{merId}}</div>
		<div class="row ">
			...
			<div class="col-md-6 mb-3 p-2">
				<div class="col-12 form-group">
					...
					@updateVal="updateVal2" ref = "defaultFdStatus"/>
				</div>
				<div class="col-12 form-group">
					...
					@updateVal="updateVal2" ref = "defaultTimeOutFdStatus"/>
				</div>
...
import { resetDefaultVal , firstUpperCase , isEmpty } from "@/utils/util" ;
import { getRiskMerIDList , getRiskStatusList } from "@/utils/dataListSA" ;
...
export default {
	...
	computed: {
	    ...
	    merIdOption : function() {
	    	let defVal = this.merId ;
	    	return {
	    		ID 			: "merId" ,
	    		func 		: getRiskMerIDList.bind(this, true ) ,
	    		addNullText : true ,
	    		selType 	: "dropdown" ,
	    		defVal 		: defVal
	    	}
	...
	methods: {
        ...
        search(){
            ...
            if( isEmpty( invalids) ) {
				...
				this.ApiUtilSA.get( "veri-id/get-store-strategy/" + merId )
				.then((res) => {
					if( res.success ) {
						let data = res.data ;
						data["storeId"] 		= merId
        ...
        save(){
			...
			this.ApiUtilSA.post("veri-id/update-store-strategy" , param )
			.then((res) => {
