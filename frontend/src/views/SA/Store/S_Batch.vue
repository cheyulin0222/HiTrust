<template>
	...
	<template v-if="!showResult">
		<div class="p-3">
			<Card title = "批次建閘道" :mainCenter="true" :titleFunc="titleFunc">
				<CardItem colMd="6">
...
export default {
				...
				colum : [
					...
					},
					{
						...
						filter : function( value , key , payment ) {
							let success = payment.success ;
							let merId = payment.merId ;
							if( success ) {
								return merId + "_PRIV.DER" ;
							} else {
								return "" ;
							}
						},
	...
	methods: {
		batchDesc(){
			...
			window.open("/" + env.AP_NAME + "/batchDesc");
		},
		updateFile(){
			let invalids = {} ;
			...
			if( isEmpty(invalids) ) {
				...
				this.ApiUtilSA.post("create-multi-stores" , form )
				.then((res) => {
					if( res.success ) {
